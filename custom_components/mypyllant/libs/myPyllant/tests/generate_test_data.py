...
from .libs.myPyllant.api import get_api_base, get_system_api_base
from .libs.myPyllant.const import DEFAULT_CONTROL_IDENTIFIER
from .libs.myPyllant.utils import add_default_parser_args
...
async def main(user, password, brand, country=None, write_results=True):
    ...
    from .libs.myPyllant.enums import DeviceDataBucketResolution
    from .libs.myPyllant.utils import datetime_format
    ...
    async with MyPyllantAPI(user, password, brand, country) as api:
        ...
        try:
            async with api.aiohttp_session.get(
                rooms_url, headers=api.get_authorized_headers()
            ) as rooms_resp:
                rooms = await rooms_resp.json()
                anonymized_rooms = _recursive_data_anonymize(copy.deepcopy(rooms), SALT)
                create_result(
                    anonymized_rooms,
                    "rooms",
                    anonymized_system_id,
